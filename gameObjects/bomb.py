...
class Bomb(GameObject):
    ...
    def update(self):
        if not self.finished:
            if not self.isExploded:
                ...
                if self.isExploded:
                    self.incUp, self.incDown, self.incLeft, self.incRight = (0,)*4
                    self.boxPosition = []
    ...
    def kill_recurse(self, i, j, move_i, move_j, depth, counter=0):
        ...
        if (i,j) in self.boxPosition:
            return counter+1

        return self.kill_recurse(i+move_i, j+move_j, move_i, move_j, depth-1, counter+1)

    def kill(self, depth):
        self.incDown   = self.kill_recurse(self.i, self.j, 1, 0, depth, -1)
        self.incUp     = self.kill_recurse(self.i, self.j,-1, 0, depth, -1)
        self.incRight  = self.kill_recurse(self.i, self.j, 0, 1, depth, -1)
        self.incLeft   = self.kill_recurse(self.i, self.j, 0,-1, depth, -1)
